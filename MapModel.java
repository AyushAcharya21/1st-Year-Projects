...
public class MapModel
{
  ...
  /**
 * This method gets the correct zone number depending on the avenue and street number of the given address
 *
 * @param address Takes in an address to get the corresponding city zone
 * @return Returns an int that represents the zone number
 */
  public static int getCityZone(String address){
    //If it's not a valid address, returns -1:
    if (!validAddress(address)){
      return -1;
    }
    //Getting the cityblock numbers in [street, avenue]:
    int cityBlock[] = getCityBlock(address); 
    int street = cityBlock[0]; //First int is street number
    int avenue = cityBlock[1]; //Second int is avenue number

    //If statements to determine the zone number that ranges from zone 0 to 3
